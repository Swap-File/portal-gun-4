...
float led_update_internal(int width_temp,int width_update_speed_temp,int overlay_temp,int total_offset)
{
	...
	static int led_width_current = 0;
	...
	if (total_offset_previous > 200 && total_offset < 200 && led_width_current == EFFECT_LENGTH) {
		//printf("RESET!\n");
		led_index = 0;
	}
	...
	if (overlay_primer == true) {
		...
		if ((width_temp <= EFFECT_LENGTH) && (width_temp >= 0)) {
	...
	if((color1_previous.r != color1.r || color1_previous.g != color1.g || color1_previous.b != color1.b) || ((led_width_requested !=0 && led_width_current == 0 ) && overlay_enabled == false)) {
	...
	if (overlay_enabled == true) {
		...
		if (ticks_since_overlay_enable > 127) { //only ramp to half brightness to save strip
			...
			led_width_current = 0 ;
			color_update_index = EFFECT_LENGTH;
			color1 = (struct CRGB) {
				.r = 0, .g = 0, .b = 0
			};
			color1_previous = color1;
			for ( int i = 0; i < EFFECT_LENGTH; i++ ) {
				main_buffer_step1[i] = (struct CRGB) {
					.r = 0, .g = 0, .b = 0
				};
	...
	if (time_this_cycle - width_update_speed_last_update > width_update_speed) {
		...
		if (led_width_requested < led_width_current && led_width_requested >= 0) {
			//printf("smaller\n" );
			led_width_current--;
			main_buffer_step1[led_width_current] = color2;
		}
		//Widen the lit section by copying the index LED color1 and time data and decrementing the index.
		else if( led_width_requested > led_width_current  && led_width_requested <= EFFECT_LENGTH) {
			//printf("bigger\n" );
			if (led_width_current == 0) {
				//starting an empty array
				main_buffer_step1[0] = color1;
				timearray[0] = timeoffset;
			} else {
				main_buffer_step1[led_width_current] = main_buffer_step1[led_width_current-1];
				timearray[led_width_current] = timearray[led_width_current - 1];
			}
			led_width_current++;
	...
	static struct CRGB handle_static = (struct CRGB) {.r = 0, .g = 0, .b = 0};
	
	int curtime = (total_offset + timeoffset + EFFECT_RESOLUTION) % EFFECT_RESOLUTION; 
	struct CRGB handle_breath;
	handle_breath.r = brightnesslookup[color_last.r][curtime];
	handle_breath.g = brightnesslookup[color_last.g][curtime];
	handle_breath.b = brightnesslookup[color_last.b][curtime];	
	...
	if (led_width_requested == 0){
		...
		if ( handle_static.r <= handle_breath.r && handle_static.g <= handle_breath.g &&  handle_static.b <= handle_breath.b){
			handle.r = handle_breath.r;
			handle.g = handle_breath.g;
			handle.b = handle_breath.b;
		} else {
			handle.r = handle_static.r;
			handle.g = handle_static.g;
			handle.b = handle_static.b;
		}
	}else{
		// pop the handle on
		handle_static.r = color_last.r;
		handle_static.g = color_last.g;
		handle_static.b = color_last.b;
	...
	for (int j = 0; j < (LED_STRIP_LENGTH-EFFECT_LENGTH); j++) {
		output_buffer[i++] = 0xFF;
		output_buffer[i++] = handle.b;
		output_buffer[i++] = handle.g;
		output_buffer[i++] = handle.r;
	}
