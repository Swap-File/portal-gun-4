...
void projector_logic_init(volatile bool *video_done_flag_p) {
    ...
    //Pi4 can run all at native 30 fps
    ...
    gstcontext_load_pipeline(GST_CACATV,        &pipeline[GST_CACATV]       ,GST_STATE_NULL,"udpsrc port=9000 caps=application/x-rtp retrieve-sender-address=false ! rtpjpegdepay ! jpegdec ! queue ! videoscale ! videoconvert ! video/x-raw,width=160,height=120,format=RGB ! videoflip video-direction=3 ! queue !  cacatv canvas-height=60 canvas-width=60  ! glupload ! glcolorconvert ! glvideoflip video-direction=1 ! glcolorscale ! video/x-raw(memory:GLMemory),width=640,height=480,format=RGBA ! glfilterapp name=grabtexture ! fakesink sync=false");
