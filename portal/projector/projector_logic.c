...
void projector_logic_init(volatile bool *video_done_flag_p){
	...
	gstcontext_load_pipeline(GST_GLMIRROR,		&pipeline[GST_GLMIRROR]		,GST_STATE_NULL,"udpsrc port=9000 caps=application/x-rtp retrieve-sender-address=false ! rtpjpegdepay ! queue ! jpegdec ! glupload ! glcolorconvert ! glvideoflip video-direction=1 ! gleffects_mirror  ! glvideoflip video-direction=3 ! video/x-raw(memory:GLMemory),width=640,height=480,format=RGBA ! glfilterapp name=grabtexture ! fakesink sync=false");
